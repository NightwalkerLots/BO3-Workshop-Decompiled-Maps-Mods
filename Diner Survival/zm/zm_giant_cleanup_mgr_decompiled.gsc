...
function private cleanup_main()
{
	...
	while(1)
	{
		...
		{
			n_override_cleanup_dist_sq = 960000;
		}
...
function do_cleanup_check(n_override_cleanup_dist)
{
	...
	if(n_time_alive < 10000)
	{
		return;
	}
	if(n_time_alive < 30000 && self.script_string !== "find_flesh" && self.completed_emerging_into_playable_area !== 1)
	{
		return;
	}
	...
	if(!b_in_active_zone)
	{
		...
		{
			n_cleanup_dist_sq = 120000;
			continue;
		}
		n_cleanup_dist_sq = 240000;
