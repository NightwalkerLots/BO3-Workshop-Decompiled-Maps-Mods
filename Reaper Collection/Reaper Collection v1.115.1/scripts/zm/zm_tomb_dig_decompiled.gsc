...
{
	level thread function_2cb1286a();
	level thread function_8993f8a6();
...
/*
	...
	Line Number: 83
*/
function function_2cb1286a()
{
	while(1)
	{
		level waittill("hash_2cb1286a", num);
		var_321d81ae = [];
		a_shovel_pos = struct::get_array("shovel_location", "targetname");
		for(i = 0; i < a_shovel_pos.size; i++)
		{
			if(!a_shovel_pos[i] function_1e978734())
			{
				var_321d81ae[var_321d81ae.size] = a_shovel_pos[i];
			}
			wait(0.1);
		}
		var_321d81ae = Array::randomize(var_321d81ae);
		for(i = 0; i < num; i++)
		{
			s_pos = var_321d81ae[i];
			m_shovel = spawn("script_model", s_pos.origin);
			m_shovel.angles = s_pos.angles;
			m_shovel SetModel("p7_zm_ori_tool_shovel");
			generate_shovel_unitrigger(m_shovel);
...
function function_1e978734()
{
	if(isdefined(level._unitriggers))
	{
		for(i = 0; i < level._unitriggers.trigger_stubs.size; i++)
		{
			...
			if(isdefined(stub.e_shovel) && DistanceSquared(self.origin, stub.origin) < 10000)
			{
				return 1;
			}
			return 0;
...
function dig_spots_respawn(a_dig_spots)
{
	while(1)
	{
		level util::waittill_any("end_of_round", "dig_spots_respawn");
		wait(2);
...
function dig_spot_spawn()
{
	...
	t_dig.targetname = "spawned_dig_spot";
	t_dig thread function_ada91eb9();
...
/*
	...
	Line Number: 551
*/
function function_ada91eb9()
{
	self endon("death");
	level waittill("hash_823f3fc8");
	if(isdefined(self))
	{
...
function waittill_dug(s_dig_spot)
{
	while(1)
	{
		...
		if(isdefined(player.dig_vars["has_shovel"]) && player.dig_vars["has_shovel"] || player == level)
		{
			if(player == level)
			{
				s_dig_spot.dug = 1;
...
function increment_player_perk_purchase_limit()
{
	if(!isdefined(self.player_perk_purchase_limit))
	{
		self.player_perk_purchase_limit = 0;
	}
...
/*
	...
	Line Number: 1354
*/
function function_8993f8a6()
{
	while(1)
	{
		level waittill("hash_296d6f52");
		level thread zm_tomb_capture_zones::recapture_round_start();
	}
}
