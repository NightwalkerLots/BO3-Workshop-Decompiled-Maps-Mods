...
function init_level_specific_audio()
{
	...
	if(zm_utility::is_Classic())
	{
		...
		level thread staff_craft_vo();
		level thread samantha_discourage_think();
		level thread samantha_encourage_think();
	}
...
function play_maxis_audio_log(v_trigger_origin, n_audiolog_id)
{
	...
	{
		var_53a95046 = soundgetplaybacktime(a_audiolog_to_play[i]);
		thread namespace_cc012897::Send(a_audiolog_to_play[i], "all", var_53a95046);
...
/*
	...
	Line Number: 1322
*/
function function_6e4732b7()
{
	level endon("hash_9d6c4da5");
	while(zm_zonemgr::any_player_in_zone("zone_bunker_6"))
	{
		level flag::set("activate_zone_bunker_6_tank");
		return;
		wait(0.05);
	}
}
...
/*
	...
	Line Number: 1343
*/
function function_72950b61()
{
	level endon("activate_zone_nml");
	for(;;)
	{
		for(i = 0; i < level.activePlayers; i++)
		{
			zone_name = zm_utility::get_current_zone();
			if(isdefined(zone_name) && IsSubStr(zone_name, "nml"))
			{
...
function play_line_on_player_character_if_present(vox_line, character_name)
{
	...
	{
		/#
			iprintln("Dev Block strings are not supported" + character_name + "Dev Block strings are not supported" + vox_line);
		#/
		var_53a95046 = soundgetplaybacktime(vox_line);
		thread namespace_cc012897::Send(vox_line, "all", var_53a95046);
...
function game_start_solo_vo()
{
	...
	if(IsArray(lines))
	{
		...
		{
			var_53a95046 = soundgetplaybacktime(lines[i]);
			thread namespace_cc012897::Send(lines[i], "all", var_53a95046);
	...
	{
		var_53a95046 = soundgetplaybacktime(a_convo[e_speaker.character_name]);
		thread namespace_cc012897::Send(a_convo[e_speaker.character_name], "all", var_53a95046);
...
function game_start_vo()
{
	...
	for(i = 0; i < a_game_start_convo.size; i++)
	{
		...
		if(line_number == 2)
		{
			...
			{
				var_53a95046 = soundgetplaybacktime(a_richtofen_lines[j]);
				thread namespace_cc012897::Send(a_richtofen_lines[j], "all", var_53a95046);
		...
		var_53a95046 = soundgetplaybacktime(a_game_start_convo["line_" + line_number][e_speaker.character_name]);
		thread namespace_cc012897::Send(a_game_start_convo["line_" + line_number][e_speaker.character_name], "all", var_53a95046);
...
function all_staffs_crafted_vo()
{
	...
	for(i = 0; i < a_convo.size; i++)
	{
		...
		var_53a95046 = soundgetplaybacktime(line);
		thread namespace_cc012897::Send(line, "all", var_53a95046);
...
function tank_left_behind_vo(e_victim, e_rider)
{
	...
	var_53a95046 = soundgetplaybacktime(str_plea_line);
	thread namespace_cc012897::Send(str_plea_line, "all", var_53a95046);
	...
	var_53a95046 = soundgetplaybacktime(str_rider_line);
	thread namespace_cc012897::Send(str_rider_line, "all", var_53a95046);
...
function round_one_end_vo()
{
	...
	for(i = 0; i < a_convo.size; i++)
	{
		...
		var_53a95046 = soundgetplaybacktime(a_convo["line_" + line_number][e_speaker.character_name]);
		thread namespace_cc012897::Send(a_convo["line_" + line_number][e_speaker.character_name], "all", var_53a95046);
		...
		if(DistanceSquared(self.origin, struct.origin) < 40000)
		{
			if(self zm_utility::is_player_looking_at(struct.origin, 0.75))
			{
				if(!(isdefined(self.dontspeak) && self.dontspeak))
				{
					...
					if(isdefined(e_richtofen))
					{
						var_53a95046 = soundgetplaybacktime(a_convo[0][e_richtofen.character_name]);
						thread namespace_cc012897::Send(a_convo[0][e_richtofen.character_name], "all", var_53a95046);
					...
					if(isdefined(struct.trigger_target) && isdefined(struct.trigger_target.is_locked))
					{
						...
						if(DistanceSquared(e_speaker.origin, e_richtofen.origin) < 2250000)
						{
							if(isdefined(e_speaker))
							{
								var_53a95046 = soundgetplaybacktime(a_convo[1][e_speaker.character_name]);
								thread namespace_cc012897::Send(a_convo[1][e_speaker.character_name], "all", var_53a95046);
					...
					if(isdefined(struct.trigger_target) && isdefined(struct.trigger_target.is_locked))
					{
						if(struct.trigger_target.is_locked == 1)
						{
							if(isdefined(e_richtofen))
							{
								var_53a95046 = soundgetplaybacktime(a_convo[2][e_richtofen.character_name]);
								thread namespace_cc012897::Send(a_convo[2][e_richtofen.character_name], "all", var_53a95046);
...
function tomb_drone_built_vo(s_craftable)
{
	...
	/#
		iprintln("Dev Block strings are not supported" + vox_line);
	#/
	...
	if(isdefined(self) && self.character_name == "Richtofen")
	{
		...
		/#
			iprintln("Dev Block strings are not supported" + self.character_name + "Dev Block strings are not supported" + vox_line);
		#/
		var_53a95046 = soundgetplaybacktime(vox_line);
		thread namespace_cc012897::Send(vox_line, "all", var_53a95046);
...
{
	return level.activePlayers.size == 1;
}
...
function say_puzzle_completion_line(n_element_enum)
{
	...
	{
		/#
			IPrintLnBold("Dev Block strings are not supported" + n_element_enum);
			return;
		#/
	}
...
function watch_one_shot_samantha_clue(str_line, str_notify, str_endon)
{
	...
	while(1)
	{
		...
		else if(isdefined(e_player) && isPlayer(e_player) && e_player.zombie_vars["zombie_powerup_zombie_blood_on"] && level flag::get("samantha_intro_done"))
		{
			level flag::wait_till_clear("story_vo_playing");
			level flag::set("story_vo_playing");
			...
			set_players_dontspeak(1);
			level.sam_talking = 1;
			var_53a95046 = soundgetplaybacktime(str_line);
			thread namespace_cc012897::Send(str_line, "all", var_53a95046);
...
function MaxisSay(vox_line, m_spot_override, b_wait_for_nearby_speakers)
{
	...
	/#
		IPrintLnBold("Dev Block strings are not supported" + vox_line);
	#/
...
function richtofenrespondvoplay(vox_category, b_richtofen_first, str_flag)
{
	...
	if(b_richtofen_first)
	{
		if(self.character_name == "Richtofen")
		{
			...
			var_53a95046 = soundgetplaybacktime(str_vox_line);
			thread namespace_cc012897::Send(str_vox_line, "all", var_53a95046);
...
function play_sam_promises_conversation(a_promises)
{
	for(i = 0; i < a_promises.size; i++)
	{
		...
		if(IsSubStr(a_promises[i], "sam_sam") || IsSubStr(a_promises[i], "samantha"))
		{
			...
			var_53a95046 = soundgetplaybacktime(a_promises[i]);
			thread namespace_cc012897::Send(a_promises[i], "all", var_53a95046);
